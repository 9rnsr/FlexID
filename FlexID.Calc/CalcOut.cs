...
namespace FlexID.Calc
{
    class CalcOut : IDisposable
    {
        ...
        private static TextWriter[] NullWriters(int n) => Enumerable.Repeat(NullWriter, n).ToArray();
        ...
        public CalcOut(InputData data, string outputPath)
        {
            ...
            // 残留放射能の出力ファイルを用意する。
            wsRete = data.OutputRetention ? CreateWriters(RetentionPath).ToArray() : NullWriters(data.Nuclides.Count);
            wsCumu = data.OutputCumulative ? CreateWriters(CumulativePath).ToArray() : NullWriters(data.Nuclides.Count);
