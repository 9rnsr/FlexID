...
ここでは、放射性物質摂取（経口摂取/吸入摂取）時の各臓器/組織の放射性物質の放射能と被ばく線量をICRP OIRシリーズの体内動態モデルに基づいて算出する方法を示す。
...
蓄積機能（`acc`, `exc`）を設定するコンパートメントでは、以下の基本式によりノードへの流入と流出を考慮した放射能の蓄積を計算する。
...
- 蓄積機能における前提条件
  ...
  流入放射能（$A$ [Bq/d]）は計算時間メッシュ期間中に一定であると仮定する。
  #figure(caption: "蓄積機能")[
...
混合機能（`mix`）を設定したコンパートメントでは、複数の流入に対して平均値を計算し、その後、流出ジャンクションから移行割合に応じて流出させる。
