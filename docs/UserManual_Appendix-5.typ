...
時系列のコンパートメントごとの計算結果を出力する。拡張子は`*.out`。ファイルの先頭部分は 表@out-header に示す構造を持つ。

#figure(caption: "出力ファイルの先頭部分")[
  #table(
    columns: 3,
    align: left,
    table.header([行番号], [出力項目], [備考]),
...
内容については1行目の「ファイル種別」に応じて 表@out-kinds に示す4つの種類がある。

#figure(caption: "出力ファイルの種別")[
  #table(
    columns: 3,
    align: left,
    table.header([ ファイル種別 ], [ 出力項目 ], [ ファイル名末尾 ]), 
...
なお、4行目の対象核種にて子孫核種が列挙されている場合、`Dose`と`DoseRate`では親核種と子孫核種の合算値を出力したものを親核種の計算結果として出力し、`RetentionActivity`と`CumulativeActivity`では親と子孫それぞれの核種ごとに計算結果を出力する。核種毎の出力は 表@out-results に示す構造を持つ。

#figure(caption: "出力ファイルの計算結果部分")[
  #table(
    columns: 2,
    align: left,
    table.header([ 行位置 ], [ 出力項目 ]),
    [ 1行目 ], [ 核種名 ],
...
FlexIDの各出力ファイルのサンプルを 図@sample1 ～ 図@sample4 に示す。

#figure(caption: "実効線量/等価線量出力ファイルのサンプル")[
  #image("images/Figure_A5-1.png")
] <sample1>

#figure(caption: "実効線量率/等価線量率出力ファイルのサンプル")[
  #image("images/Figure_A5-2.png")
] <sample2>

#figure(caption: "残留放射能出力ファイルのサンプル")[
  #image("images/Figure_A5-3.png")
] <sample3>

#figure(caption: "積算放射能出力ファイルのサンプル")[
  #image("images/Figure_A5-4.png")
] <sample4>
...
#figure(caption: [残留放射能出力ファイルにおける`Urine`と`Faeces`の出力例])[
  #image("images/Figure_A5-5.png")
] <rete-excreta>
